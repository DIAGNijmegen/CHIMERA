...
def build_datasets(csv_splits, model_type, batch_size=1, num_workers=2, train_kwargs={}, val_kwargs={}, loss_fn=None):
    """
    ...
    for k in csv_splits.keys(): # ['train', 'val', 'test']
        ...
        # For ranking-based losses (cox, cox_time, rank), we need batch_size > 1 for meaningful loss computation
  
        if dataset_kwargs.get('bag_size', -1) > 0:
            # Fixed bag size - can use larger batch
            effective_batch_size = batch_size
        else:
            # Variable bag size - use batch_size=1 but adjust gradient accumulation for ranking losses
            if loss_fn in ['cox', 'cox_time', 'rank']:
                # For ranking losses, we'll use batch_size=1 with sample accumulation in trainer
                effective_batch_size = 1
                print(f"Variable bag size with {loss_fn} loss: using batch_size=1 with sample accumulation.")
            else:
                effective_batch_size = 1

        dataloader = DataLoader(dataset, batch_size=effective_batch_size,
                                shuffle=dataset_kwargs['shuffle'], num_workers=num_workers)
        dataset_splits[k] = dataloader
        print(f'split: {k}, n: {len(dataset)}, batch_size: {effective_batch_size}')
...
def main(args):
    ...
    # Auto-adjust gradient accumulation for ranking losses with variable bag sizes
    if (args.loss_fn in ['cox', 'cox_time', 'rank'] and 
        args.bag_size <= 0 and args.batch_size > 1):
        print(f"Auto-adjusting sample accumulation for {args.loss_fn} loss with variable bag sizes:")
        print(f"  Original: batch_size={args.batch_size}, accum_steps={args.accum_steps}")
        # Increase accumulation to simulate the requested batch size
        args.accum_steps = max(args.accum_steps, args.batch_size)
        print(f"  Adjusted: batch_size=1, accum_steps={args.accum_steps}")
        print(f"  Note: Cox losses require multiple samples, so loss is computed every {args.accum_steps} steps.")
    ...
    # Also save dumps as CSV files for easier viewing
    for split_name, split_dumps in fold_dumps.items():
        if split_dumps and 'all_risk_scores' in split_dumps:
            # Create CSV with predictions vs ground truth
            if 'all_predicted_times' in split_dumps:
                # Time prediction model - compare predicted time vs actual time
                split_df = pd.DataFrame({
                    'sample_id': split_dumps['sample_ids'],
                    'predicted_time': split_dumps['all_predicted_times'],
                    'actual_time': split_dumps['all_event_times'],
                    'risk_score': split_dumps['all_risk_scores']  # Also include risk for c-index
                })
            else:
                # Risk prediction model - use risk scores
                split_df = pd.DataFrame({
                    'sample_id': split_dumps['sample_ids'],
                    'risk_score': split_dumps['all_risk_scores'],
                    'actual_time': split_dumps['all_event_times']
                })
            
            csv_path = j_(args.results_dir, f'{split_name}_predictions_vs_gt.csv')
            split_df.to_csv(csv_path, index=False)
            print(f"Saved {split_name} predictions vs ground truth to: {csv_path}")
...
### model args ###
parser.add_argument('--model_type', type=str, choices=['ABMIL', 'ABMIL_Time'], default='ABMIL_Time',
                    help='type of model')
...
parser.add_argument('--loss_fn', type=str, default='cox_time', choices=['nll', 'cox', 'cox_time', 'sumo', 'ipcwls', 'rank'],
...
parser.add_argument('--task', type=str, default='prostate_bcr_survival')
...
parser.add_argument('--patch_mag', type=int, default=20,
                    help='Patch magnification level (e.g., 20 for 20x). Default: 20 (UNI standard)')
parser.add_argument('--patch_size', type=int, default=224,
                    help='Patch size in pixels (e.g., 224). Default: 224 (UNI standard)')
...
if __name__ == "__main__":
    ...
    for src in args.data_source: 
        ...
        if (mag < 0 or patch_size < 0):
            mag, patch_size = args.patch_mag, args.patch_size
            print(f"Using provided patch parameters: mag={mag}x, patch_size={patch_size}")
