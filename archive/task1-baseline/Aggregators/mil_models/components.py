...
def process_surv_time(predicted_time, label, censorship, loss_fn=None):
    results_dict = {'predicted_time': predicted_time}
    log_dict = {}
    
    # Always compute risk score for c-index calculation (negative time = higher risk for longer survival)
    results_dict['risk'] = -predicted_time

    if loss_fn is not None and label is not None:
        surv_loss_dict = loss_fn(logits=predicted_time, times=label, censorships=censorship)
        loss = surv_loss_dict['loss']
        log_dict['surv_loss'] = surv_loss_dict['loss'].item()
        log_dict.update(
            {k: v.item() for k, v in surv_loss_dict.items() if isinstance(v, torch.Tensor)})
