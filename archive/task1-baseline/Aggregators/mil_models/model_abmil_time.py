...
from .components import Attn_Net, Attn_Net_Gated, create_mlp, process_surv_time
from .model_configs import ABMILConfig


class ABMIL_Time(nn.Module):
    def __init__(self, config, mode):
        ...
        # Time prediction head - outputs positive time values
        self.time_predictor = nn.Sequential(
            nn.Linear(config.embed_dim, config.embed_dim // 2),
            nn.ReLU(),
            nn.Dropout(config.dropout),
            nn.Linear(config.embed_dim // 2, 1),
            nn.Softplus()  # Ensures positive output for time
        )
    ...
    def forward_no_loss(self, h, attn_mask=None):
        ...
        # Predict time instead of logits
        predicted_time = self.time_predictor(M)

        out = {'predicted_time': predicted_time, 'attn': A, 'feats': h, 'feats_agg': M}
    ...
    def forward(self, h, model_kwargs={}):
        ...
        if self.mode == 'classification':
            raise NotImplementedError("Time prediction model only supports survival mode")
            ...
            results_dict, log_dict = process_surv_time(predicted_time, label, censorship, loss_fn)
        else:
