...
class WSIClassificationDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        clinical_row = self.data_df.loc[self.get_sample_id(idx), self.clinical_cols]
        
        clinical_feats = torch.tensor(clinical_row.astype(float).values, dtype=torch.float32)

        out['clinical'] = clinical_feats
