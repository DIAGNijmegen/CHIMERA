...
parser.add_argument('--opt', type=str,
                    choices=['adamW', 'sgd','RAdam'], default='adamW')
...
parser.add_argument('--clinical_in_dim', type=int, default=12,
                    help="Number of clinical features used in ABMIL_Fusion")
...
parser.add_argument('--model_type',
                    type=str,
                    choices=['H2T', 'ABMIL', 'ABMIL_Fusion', 'TransMIL', 'SumMIL', 'OT', 'PANTHER', 'ProtoCount', 'DeepAttnMIL', 'ILRA'],
                    required=True)
...
if __name__ == "__main__":
    ...
    if args.task not in label_dicts:
        raise ValueError(f"Unknown task '{args.task}'. Available: {list(label_dicts.keys())}")
    ...
    # Parsing fold name like 'fold_0'
    folder_name = os.path.basename(args.split_dir)
    parts = folder_name.split('_')

    if len(parts) == 2 and parts[0] == 'fold':
        split_num = parts[1]
        args.split_k = split_num 
        args.split_name_clean = 'fold'
    else:
        raise ValueError(f"Unrecognized split folder format: {folder_name}")
