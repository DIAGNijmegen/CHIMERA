...
class WSISurvivalDataset(Dataset):
    ...
    def __init__(self,
        ...
        for src in data_source:
            ...
            assert basename in ['feats_h5', 'feats_pt', 'features']
            # Determine file format based on directory name or check for .h5 files
            if basename == 'feats_h5':
                self.use_h5 = True
            elif basename == 'feats_pt':
                self.use_h5 = False
            else:  # basename == 'features'
                # Check if directory contains .h5 or .pt files to determine format
                sample_files = os.listdir(src)
                if any(f.endswith('.h5') for f in sample_files):
                    self.use_h5 = True
                else:
                    self.use_h5 = False
